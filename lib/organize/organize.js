...
return 0; //inside 
    ...
    { 
        ...
        circ.dragstart = function() {
            ...
            {
                entPos = new Object();
                entPos.x = that.svgElements[circlePositionInArray].entities[i].svg.attr('x'); 
                entPos.y = that.svgElements[circlePositionInArray].entities[i].svg.attr('y'); 
    ...
    {        
        ...
        var rect = this.draw.rect(ent.attr('width')+5,ent.attr('width')+5).attr({ fill: 'none' }).stroke({ width: 0.5,color:'blue' });
        ...
        var difX = this.draggedEntityInitialX-ent.attr('x');
        var difY = this.draggedEntityInitialY-ent.attr('y');    
    ...
    {
        // Creating the entity
        var ent = this.draw.image(ousideEntity.imageURL, 50, 50)        
        ent.attr({ x: ousideEntity.x, y: ousideEntity.y });
        ent.draggable();
        var that = this;
        ent.dragstart=function() {this.isDraggingEntity=1;
            ...
            that.draggedEntityInitialX = ent.attr('x');
            that.draggedEntityInitialY = ent.attr('y');
        };
        ent.click(function() {
            var divSVG = that.svgDiv;    
            divSVG.focus();
    ...
    {
        var x = this.draggedEntity.attr('x');
        var y = this.draggedEntity.attr('y');
    ...
    ,updateDraggedEntityProperties :  function()
    {
        ...
        for(i=0; i<this.svgElements.length;i++)
        {
            ...
            {          
                if(this.svgElements[i].entities[j].svg == ent)
                {
                    this.svgElements[i].entities[j].x = ent.attr('x');
                    this.svgElements[i].entities[j].y = ent.attr('y');
                }  
            }
        }
    }
