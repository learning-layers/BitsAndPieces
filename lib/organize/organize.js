...
Organize.prototype = {
    ...
    labelDistanceCy:0,
    labelCounter:0,
    // These elements are part of an instance, not prototype and shared among instances
...
return 0; //inside 
    ...
    ,removeCircle: function(circle, label)
    {
        this.removeCircleFromArray(circle);
        circle.remove();
        label.remove();
    ...
    {
        this.removeEntityFromAllCircles(entity);
        entity.remove();
        if ( this.selectedEntityRect!=null ) {
    ...
    ,removeCircleAndEntity : function(event)
    {
        ...
        if (event.keyCode==8 || event.keyCode==46) // if bakspace (8) or delete (46) keys are pressed
        {
            ...
            if ( !this.getAreChangesEnabled() ) return true;
            ...
            { 
                ...
                this.removeCircle(this.selectedCirc, this.selectedLabel);
            }
    ...
    ,quietUnselectLabelAndInside: function() {
        this.insideLabel = 0;
        this.currentLabel = null;
    }
    ...
    { 
        ...
        var el=this.svgDiv;  
        this.LEFTOFFSET = $(el).find('svg').offset().left;
        this.TOPOFFSET = $(el).find('svg').offset().top;
    }
    ...
    ,changeCircleAndLabel : function(circ,text,circIntegration)
    {
        ...
        this.log("Received parameters:");

        this.log("cx: "+circIntegration.cx);
        this.log("cy: "+circIntegration.cy);
        this.log("rx: "+circIntegration.rx);
        this.log("ry: "+circIntegration.ry);
        this.log("LabelX: "+circIntegration.LabelX);
        this.log("LabelY: "+circIntegration.LabelY);
        this.log("LabelX: "+circIntegration.Label);
    ...
    { 
        ...
        // This should help differenciate
        ...
        if ( this.getAreChangesEnabled() ) {
            circ.draggable(this.DRAG_CONSTRAINTS);
        ...
        circ.click(function(clickEvent) {
            that.updateOffsets();
            var smallestCircle = that.getSmallestCircleAtCoordinates(clickEvent.pageX-that.LEFTOFFSET, clickEvent.pageY-that.TOPOFFSET);

            // If there is a smaller circle at the coordinates clicked
            // and it is not the current one, then that one is clicked
            // instead
            if ( smallestCircle === null || smallestCircle.svg === circ ) {
                that.clickCircle(circ, text);
            } else {
                // Force the small circle in front
                smallestCircle.svg.before(circ);
                smallestCircle.label.svg.before(text);
                that.clickCircle(smallestCircle.svg, smallestCircle.label.svg);
            }
        });
        ...
        circ.dragstart = function() {
            ...
            that.log("Size of ArrayOfOverlappingCircles: "+ArrayOfOverlappingCircles.length);
        };
        ...
        circ.dragend = function(delta, event) {
            ...
            //circ.draggable({minX:0,minY:0,maxX:800,maxY:600 });        
            if ( that.getAreChangesEnabled() ) {
                circ.draggable(that.DRAG_CONSTRAINTS);
            }
            ...
            for (var mm=0; mm<ArrayOfOverlappingCircles.length;mm++)
            {
                ...
                {
                    ArrayOfOverlappingCircles[mm].entities[pp].x = ArrayOfOverlappingCircles[mm].entities[pp].svg.transform().x;  
                    ArrayOfOverlappingCircles[mm].entities[pp].y = ArrayOfOverlappingCircles[mm].entities[pp].svg.transform().y;
                } 
            ...
            if(this.svgElements[i].svg==circ) // the cirrent circle is found         
            {
                ...
                {   this.log("checking entity at position: "+j);
                    ...
                    for(k=0;k<this.svgElements.length;k++)
                    {
                        ...
                        this.log("Trying to find that entity at circle: "+k);
                        if((k!=i)&&(this.isEntityInACircleInArray(k,ent)!=-1))
                        {
                            this.log("found new circle!");
                            inserted = false; 
    ...
    ,drawTag : function(xCirc,yCirc,x,y,initialLabelText,rxCirc,ryCirc)
    {
        ...
        text.dblclick(function(event) {
            ...
            if ( !that.getAreChangesEnabled() ) return false;
            ...
            for(var i=0;i<that.svgElements.length;i++)
                {
                    if(that.svgElements[i].label.svg== text)
                    {
                        ...
                        that.log("Circle found!!!");
                        break;
                    }
                }

            //Firing the ChangeCircleLabel event (due to change on the label position)
            var divSVG = that.svgDiv;
            var entIntegration = that.createObjectForEvent(1,circL,text);
            var customevEnt = $.Event('ChangeCircleLabel', { 'detail': entIntegration });
            $(divSVG).trigger(customevEnt);
        });
        text.click(function() {
            if ( !that.getAreChangesEnabled() ) {
            ...
            if(difX==0 && difY==0)  // If the coordinates of the Label did not change...
                ...
                {
                    // This is being handled by DoubleClick now
                    // This one is kept not to fire the ChangeCircle event
                    that.insideLabel = 1; // This one is reset back elsewhere
                    return;
                ...
                var divSVG = that.svgDiv;    
                var entIntegration = that.createObjectForEvent(1,circL,text);     
                var customevEnt = $.Event(eventName, { 'detail': entIntegration });
                //divSVG.dispatchEvent(customevEnt);
                $(divSVG).trigger(customevEnt);
                that.log("Change circle fired!");
        });
    ...
    {        
        ...
        if( ( difX==0 && difY==0 )
                || ( !this.getAreChangesEnabled() )
                )  // If the coordinates of the entity did not change...
        ...
        {  
            ...
            //divSVG.dispatchEvent(customevEnt);              
            $(divSVG).trigger(customevEnt);
            this.log('ClickEntity Triggered ');
        } 
    }
    ...
    ,loadOrganizeCanvas : function(divID)
    {
        ...
        this.outsideCircleElements = new Array();
        ...
        // Handle scroll of container div and update offsets
        $(this.svgDiv).on('scroll', function() {
    ...
    ,findParent: function(element, match) {
        this.log('findParent', element, match);
        while(element && element !== match) {
    ...
    ,createAndDropSvgEntity : function(ousideEntity)
    {
        ...
        if ( this.getAreChangesEnabled() ) {
            ent.draggable(this.DRAG_CONSTRAINTS);
        }
        var that = this;
        ...
        ent.mouseenter(function(e) {
            var g = $(that.findParent(e.target, ent.node));
            var label = g.attr('label') || "";
            that.log('mouseenter then', e, g, label);
            g.find('tspan').text(label);
        });
        
        ent.mouseleave(function(e) {
            var g = $(that.findParent(e.target, ent.node));
            var label = g.attr('label') || "";
            that.log('mouseleave', e, g, label);
            g.find('tspan').text(
                label.length > 15 
                ? label.substring(0,15) + " ..." 
                : label);
    ...
    {
        for(var j=0;j<this.outsideCircleElements.length;j++)
        {
            this.removeEntity(this.outsideCircleElements[j]);
        }

        var length = this.svgElements.length;
        for (var i=0;i<length;i++) {
            this.removeCircle(this.svgElements[0].svg, this.svgElements[0].label.svg);
        }

        this.draw.clear();
        this.svgElements = [];
        this.outsideCircleElements = [];
        // Set currentLabel to null or it will cause errors
        this.currentLabel = null;
    },setChangesDisabled : function() {
        this.changesEnabled = false;
    },setChangesEnabled : function() {
        this.changesEnabled = true;
    },getAreChangesEnabled : function() {
        if ( false === this.changesEnabled ) {
            return false;
        }

        return true;
    },getSmallestCircleAtCoordinates: function(x, y) {
        var smallestArea = null;
        var smallestCirclePosition = null;

        for(var i=0;i<this.svgElements.length;i++) {
            var isInside = this.insideCircle(x, y, this.svgElements[i].svg);
            if ( isInside ) {
                var currentArea = Math.PI * this.svgElements[i].svg.attr('rx') * this.svgElements[i].svg.attr('ry');

                if ( smallestArea === null ) {
                    smallestArea = currentArea;
                    smallestCirclePosition = i;
                } else if ( smallestArea >= currentArea ) {
                    smallestArea = currentArea;
                    smallestCirclePosition = i;
                }
            }
        ...
        return (smallestCirclePosition !== null) ? this.svgElements[smallestCirclePosition] : null;
    },
    log: function() {
        if ( this.loggingEnabled && window.console && console.log ) {
